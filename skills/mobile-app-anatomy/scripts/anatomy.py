...
def exclusion_reason(rel: str, path: Path, output: Path, include_dirs: Iterable[str] = ()) -> str | None:
    parts = set(Path(rel).parts)
    if any(part in EXCLUDED_DIR_NAMES for part in parts) and not file_is_forced_included(rel, include_dirs):
        return "third-party dependency, cache, or build output directory"
    try:
...
def scan_repo(
    ...
    for base, dirs, files in os.walk(repo):
        ...
        for d in dirs:
            ...
            try:
                ...
                continue
            if d in EXCLUDED_DIR_NAMES and not include_intersects(candidate_rel, include_dirs):
                continue
            kept_dirs.append(d)
        dirs[:] = kept_dirs
    ...
    for rel in sorted(files_by_rel):
        ...
        reason = exclusion_reason(rel, path, output, include_dirs)
    ...
    ledger = {
        ...
        "excluded_directory_patterns": [
            {
                "pattern": name,
                "reason": "third-party dependency, cache, editor metadata, or build output directory",
            }
...
def command_init(args: argparse.Namespace) -> int:
    ...
    if (output / "machine" / "ledger.json").exists() and not args.force:
        ...
        return 2
    scan = scan_repo(repo, output, args.chunk_lines, None, args.include_dir, args.runtime_mode)
...
def command_claim(args: argparse.Namespace) -> int:
    ...
    if not unit:
        print("Unknown unit.", file=sys.stderr)
        return 2
    if unit.get("excluded"):
