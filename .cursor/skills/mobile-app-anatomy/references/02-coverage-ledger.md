...
Unit IDs are deterministic for a unit type, repository-relative path, and current line interval. They can disappear after `init --force`, file removal, exclusion changes, or chunk-boundary changes. Only claim IDs present in the current `machine/ledger.json` or returned by the current `claim-next` call. An `Unknown unit` result means the work was not claimed and must not be counted as covered. Reload and claim again.
